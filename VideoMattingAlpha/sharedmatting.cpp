...
{
	inputMat.copyTo(mat_pImg);
	if (!mat_pImg.data)
	{
		cout << "Loading Image Failed!" << endl;
		exit(-1);
	}
	height = mat_pImg.rows;
	width = mat_pImg.cols;
	step = mat_pImg.step;
	channels = mat_pImg.channels();
	data = mat_pImg.ptr<uchar>(0);
	...
	mat_matte = cv::Mat::zeros(height, width, CV_8UC1);
}

void SharedMatting::loadImage(char * filename)
{
	mat_pImg = cv::imread(filename);
	if (!mat_pImg.data)
	{
		cout << "Loading Image Failed!" << endl;
		exit(-1);
...
void SharedMatting::expandKnown()
{
	...
	int kc2 = kC * kC;
	vp.clear();
	int s = mat_trimap.step;
	int c = mat_trimap.channels();
	uchar * d = mat_trimap.ptr<uchar>(0);
	for (int i = 0; i < height; ++i)
	{
...
{
	cv::imwrite(filename, mat_matte);
}

void SharedMatting::getMatte(cv::Mat &alpha){//added by jacques
	mat_matte.copyTo(alpha);
}

void SharedMatting::getMatte()
{
	int h = mat_matte.rows;
	int w = mat_matte.cols;
	int s = mat_matte.step;
	uchar* d = mat_matte.ptr<uchar>(0);
	for (int i = 0; i < h; ++i)
	{
		for (int j = 0; j < w; ++j)
		{
